...
class MongoConnector:
    ...
    def insert_one_document(self, mongo_media_item: "RawMediaMongoDocument", data_stage: DataStage) -> None:
        if data_stage == DataStage.RAW:
            _collection = self._db[self._raw_colname]
            _collection.insert_one(mongo_media_item.model_dump(by_alias=True, exclude=["id"]))
        elif data_stage == DataStage.CLEAN:
            pass
        else:
            raise ValueError(f"Invalid data stage: {data_stage}")
