...
def unpack_parameters(**kwargs):
    # Access parameters from dag_run.conf, providing defaults
    subject = kwargs["dag_run"].conf.get("subject", "Default Subject")
    num_pages = kwargs["dag_run"].conf.get("num_pages", 1)
    last_page = kwargs["dag_run"].conf.get("last_page", None)  # Example of a potentially optional parameter
    ...
    # Return the dictionary. This will be pushed to XCom automatically.
    return params  # Implicit XCom push
